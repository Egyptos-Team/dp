...
export default function FixedInfo() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const response = await fetch("https://egyptos.runasp.net/api/AreaTypes/Get/1");
    ...
    <div className="min-h-screen flex flex-col mx-auto container p-4 md:p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 my-6">
        {visibleImages.map((src, index) => (
          ...
          >
            {src && <Image src={`https://egyptos.runasp.net/${src}`} unoptimized alt={`Thumbnail ${index + 1}`} fill className="object-fill opacity-90" />}
          </div>
        ))}
