    ...
    async function fetchData() {
      try {
        const response = await axios.get(
          "https://egyptos.runasp.net/api/AreaTypes/Get/2"
        );
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center py-7">
        <div>
          ...
          <span className="text-xl text-gray-400">
            Welcome to traps historic area
          </span>
        </div>
        <div>
          <select
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col justify-end items-start text-white py-6 px-3 rounded-2xl">
              ...
              <div className="flex justify-between items-center w-full mt-3">
                <Link href={`/explore/artifacts/${item.id}`} className="flex items-center">
            ...
            {isMapOpen && (
              <MapModal location={mapLocation} onClose={() => setIsMapOpen(false)} />
            )}
          </div>
        ))}
      </div>
      ...
      {filteredData.length > count && (
        <div className="flex justify-center mt-10">
          <button
            ...
            className="px-6 py-3 bg-[#020032] text-white rounded-md hover:bg-[#020032] transition duration-300"
          >
            Explore More
          </button>
        </div>
