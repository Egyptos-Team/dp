  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(
        `https://egyptos.runasp.net/api/EventType/Update/${id}`,
        { id, name },
        {
          headers: {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-[10000]">
      <div className="bg-white p-6 rounded-lg w-full max-w-sm relative text-black">
        ...
        <h2 className="text-xl font-bold mb-4">Update Event Type</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block mb-1 font-medium">Event Type ID</label>
            <input
              type="number"
              name="id"
              value={formData.id}
...
export default UpdateEventTypeModal;
