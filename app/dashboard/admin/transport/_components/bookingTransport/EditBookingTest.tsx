...
export default function RideForm({ item, token }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "https://egyptos.runasp.net/api/BookingPrivateTransports/Create",
        {
          method: "POST",
          headers: {
          ...
          <div className="w-[70%]   ">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {/* لو بعت داتا هتظهر */}
                {props.vehicleID && (
                  <option value={props.vehicleID}>{props.vehicleName}</option>
                )}
                ...
                dataVehicle.privateTransports.length === 0 ? (
                  ...
                  <>
                    ...
                    {dataVehicle.privateTransports.map(
